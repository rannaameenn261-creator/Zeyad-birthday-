...
musicBtn.addEventListener('click', async () => {
  if (song.paused) {
    try { await song.play(); musicText.textContent = 'Pause our song'; }
    catch(e) { musicText.textContent = 'Add song.mp3'; }
  } else {
    song.pause(); musicText.textContent = 'Play our song';
  }
});
...
function burst(){
  for(let i=0;i<30;i++){
    setTimeout(()=>{
      ...
      requestAnimationFrame(()=>{h.style.transform=`translate(${(Math.random()-.5)*350}px, ${(Math.random()-.5)*350}px) rotate(${Math.random()*360}deg)`;h.style.opacity=0});
